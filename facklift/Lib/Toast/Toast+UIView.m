...
#define kDefaultPosition            @"top"

#define kImageWidth                 80.0
#define kImageHeight                80.0

#define kActivityWidth              340.0
#define kActivityHeight             568.0
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position immediately:(BOOL)immediately {
    if (immediately) {
        ...
        id point = position == nil ? kDefaultPosition : position;
...
- (CGPoint)getPositionFor:(id)point toast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        if( [point caseInsensitiveCompare:@"top"] == NSOrderedSame ) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + kVerticalPadding+64);
    ...
    DDLogInfo(@"Error: Invalid position for toast.");
...
- (UIView *)makeViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (kDisplayShadow) {
        ...
        [wrapperView.layer setShadowOffset:CGSizeMake(1.0, 1.0)];
    }
    ...
    // 给 Toast 增大 zPosition 可以避免它被页面的3D动画遮挡。
    wrapperView.layer.zPosition = 150;
    return wrapperView;
}
